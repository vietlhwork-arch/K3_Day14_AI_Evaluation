...
class RAGASEvaluator:
    ...
    def evaluate_relevance(self, answer: str, question: str) -> float:
        ...
        if not question_tokens:
            return 1.0
        answer_tokens = _tokenize(answer)
        return float(len(answer_tokens & question_tokens)) / len(question_tokens)
    ...
    def evaluate_completeness(self, answer: str, expected: str) -> float:
        ...
        if not expected_tokens:
            return 1.0
        answer_tokens = _tokenize(answer)
        return float(len(answer_tokens & expected_tokens)) / len(expected_tokens)
    ...
    def evaluate_context_recall(self, contexts: list[str], expected: str) -> float:
        ...
        return float(len(expected_tokens & union_tokens)) / len(expected_tokens)
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            relevant_flags.append(1 if overlap >= relevance_threshold else 0)
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = f"Question: {question}\nAnswer: {answer}\nRubric: {rubric}\nPlease score according to the rubric and return JSON format."
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        regressions = []
        for name, key in metrics_to_check:
            n_val = new_report.get(key, 0.0)
            b_val = base_report.get(key, 0.0)
            if b_val - n_val > 0.05:
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        """
        if failure.faithfulness < 0.3 and failure.faithfulness <= failure.relevance and failure.faithfulness <= failure.completeness:
            return "Context is missing or irrelevant — improve retrieval"
        if failure.relevance < 0.3 and failure.relevance <= failure.faithfulness and failure.relevance <= failure.completeness:
            return "Answer does not address the question — improve prompt clarity"
        if failure.completeness < 0.3 and failure.completeness <= failure.relevance and failure.completeness <= failure.faithfulness:
    ...
    def generate_improvement_suggestions(
        ...
        if counts.get("irrelevant", 0) > 0:
            suggestions.append("Improve prompt clarity to prevent irrelevant answers")
        ...
        if counts.get("off_topic", 0) > 0:
            suggestions.append("Enhance intent detection to stay on topic")
            
        defaults = [
            "Add few-shot examples showing complete answers to improve completeness",
            "Tune retrieval hyperparameters for better context precision",
            "Review source documents to ensure necessary information is present"
        ]
